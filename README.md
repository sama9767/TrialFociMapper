...
## Description
All [ClinicalTrials.gov](https://classic.clinicaltrials.gov/) data is available in the  schema of the [AACT database](https://aact.ctti-clinicaltrials.org/). The Browse Conditions table in the AACT database contains data regarding the conditions studied in the trial.
When data submitters provide this information to ClinicalTrials.gov about a study, they are encouraged to use Medical Subject Heading (MeSH) terminology. The browse_conditions table contains MeSH terms generated by an algorithm run by the National Library of Medicine (NLM). The NLM algorithm is run nightly on all studies in the ClinicalTrials.gov database and sources the most up-to-date information from the study record.

The TrialFociMapper package retrieves the browse_conditions table for a given trial and assigns major [MeSH headings](https://meshb.nlm.nih.gov/treeView) using the [NLM descriptor data](https://meshb.nlm.nih.gov/treeView) as a reference. There are two functions in the package:
1) `generate_foci`: It leverages the information provided by data submitters to ClinicalTrials.gov and generates major foci for each trial.
2) `assign_therapeutic_focus`: Additonal function to assign a trial with multiple therapeutic foci with a single therapeutic focus using a disease-centric approach described below.
